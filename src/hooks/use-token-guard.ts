...
export default function useTokenGuard() {
  ...
  const { setGatekeeperNetwork } = useGatekeeperNetwork();
  ...
  useEffect(() => {
    (async () => {
      if (!tokenGuardState?.gatekeeperNetwork || !wallet || !wallet.publicKey || !setGatekeeperNetwork) return;
      ...
      setGatekeeperNetwork(tokenGuardState.gatekeeperNetwork);
    })();
  }, [tokenGuardState, setGatekeeperNetwork, setGatewayToken, wallet, wallet?.publicKey])
